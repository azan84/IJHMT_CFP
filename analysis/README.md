...
|---|---|---|
| `dataset_ledger_unitcell.csv` | one row per finished case (162 rows at this date, 6 September 2026: 90 solved on the workstation, 72 on the second machine, which continued the shared list unattended; 48 cases accepted, 34 of them the calibration set): bypass fractions at the leading edge, mid-length and trailing edge, the effective fraction, the length-averaged and local Nusselt numbers, base and wall temperatures, thermal resistance, pressure drop, pumping power, closures, stop type, acceptance flags and the columns the fitting script reads | `../unit_cell_campaign/post_campaign.py` |
...
not in the repository. The second machine has continued the shared list unattended since; 15 cases remain (the grid study, G001 and
G002, and the rest of the withheld-coolant, withheld-load and cross-combination sets). The calibration set has
been complete since 5 September, so the fitted coefficients of Table~coefficients are unchanged by the new
cases; `tab_statistics.tex` now carries partial statistics for the other partitions as they complete.

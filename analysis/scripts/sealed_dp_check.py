...
A=os.environ.get("ANALYSIS_DIR"); CASES=os.environ.get("CASES_DIR",os.path.join(C,"cases")); UC=os.environ.get("UNIT_CELL_DIR",C)   # repository layout when set
sys.path.insert(0,UC); import unit_cell as u
d=pd.read_csv(os.path.join(A,"dataset_ledger_unitcell.csv") if A else os.path.join(C,"dataset_ledger_unitcell.csv")); s=d[(d.OR==0)&(d.fluid=="FC-40")&(d.partitions.str.contains("calibration"))].sort_values("Re_ch")
rows=[]
for r in s.itertuples():
    m=json.load(open(os.path.join(CASES,r.case_id,"case_meta.json"))); Hf=m["H_fin_m"]; Dh=m["D_h_m"]; uch=m["u_in_m_s"]*(u.S/2+u.TF/2)/(u.S/2)
...
o=pd.DataFrame(rows); o.to_csv(os.path.join(A,"sealed_dp_check.csv") if A else os.path.join(ROOT,"audit/sealed_dp_check.csv"),index=False); print(o[["case_id","Re_ch","dp_cfd_Pa","dp_SL_Tin_Pa","ratio_Tin","ratio_Tbulk","ratio_Tfilm"]].to_string(index=False,float_format=lambda x:"%.4g"%x))

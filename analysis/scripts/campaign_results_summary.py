...
ADIR=os.environ.get("ANALYSIS_DIR")   # when set: ledger, statistics and the summary live there (repository layout)
LEDGER=os.path.join(ADIR,"dataset_ledger_unitcell.csv") if ADIR else os.path.join(C,"dataset_ledger_unitcell.csv"); AUD=ADIR if ADIR else os.path.join(ROOT,"audit")
...
d=pd.read_csv(LEDGER); d["acc"]=d.accepted.astype(str)=="True"; d["conv"]=d.converged.astype(str)=="True"; d["env"]=d.passed_validity_envelope=="y"
cal=d[d.partitions.str.contains("calibration")]; A=cal[cal.acc]; L=[]
def p(x): L.append(x)
p("# Campaign results summary (%d finished cases in the ledger)\n"%len(d)); p("Source: %s unless stated.\n"%("analysis/dataset_ledger_unitcell.csv" if ADIR else "cfd/unit_cell_campaign/dataset_ledger_unitcell.csv"))
...
fm=pd.read_csv(os.path.join(AUD,"feasibility_map.csv")); ff=fm[fm.feasible.astype(str).isin(["True","y"])]
...
t=pd.read_csv(os.path.join(AUD,"refit_stats.csv")).set_index("partition"); c=t.loc["calibration"]

...
Round 1 (`audit/round1_report.md` in this directory) found five blocking and six non-blocking items. All were
addressed: push paths and return codes; numpy declared and checked; OpenFOAM located through PATH,
OPENFOAM_BASHRC or the usual install paths in every script; ranks fixed at 8; stop markers and logs of every
earlier pass renamed with the pass index before a continuation; the dead branch and unused regex removed and the
trailing-edge boxes taken from the case's own leading-edge boxes; --test needs no prompt; the repository's
.gitignore (copy in ../gitignore_of_repo.txt) keeps built cases out and the run log in. The push path was exercised
against a local bare repository. Re-check every round-1 item against the current files, then the rest of the
instructions, and give the verdict block again.

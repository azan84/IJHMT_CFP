...
- Dynamic Parallel Job Scheduling & Warm-Start Sequence Optimization
- Automatic Case Generation, FVM Meshing & Parallel Solver Execution
- Live Post-Processing (Bypass Fraction, Thermal Resistance, Pressure Drop, FOM)
- Automatic GitHub Synchronization (Pull & Push live results)
"""
...
from env_check import verify_environment, get_system_hardware, find_openfoam_bashrc
...
def parse_args():
    ...
    parser.add_argument("--case-id", type=str, default=None, help="Run a single specific case by ID (e.g. DOE_Case_01)")
    return parser.parse_args()

def interactive_setup(hw, args):
    if args.non_interactive:
        threads = args.threads or max(1, int(hw["cpu_threads"] * 0.75))
        ram = args.ram or max(4.0, round(hw["total_ram_gb"] * 0.75, 1))
        auto_push = args.auto_push
        exec_mode = 1
        return threads, ram, exec_mode, auto_push
    ...
    # 1. CPU Allocation
    default_threads = max(1, int(hw["cpu_threads"] * 0.75))
    try:
        th_input = input(f"--> Enter CPU threads to allocate [1-{hw['cpu_threads']}, Default: {default_threads} (75%)]: ").strip()
    ...
    # 2. RAM Allocation
    default_ram = max(4.0, round(hw["total_ram_gb"] * 0.75, 1))
    try:
    ...
    print(f"    [1] Balanced Auto-Optimizer (Runs cases with {threads} MPI ranks per case) [RECOMMENDED]")
    print(f"    [2] High-Throughput Batch (Runs {max(1, threads//4)} concurrent cases with 4 MPI ranks each)")
    print(f"    [3] Conservative Single-Job (Runs 1 sequential case at a time)")
    mode_input = input("--> Select Execution Mode [1/2/3, Default: 1]: ").strip()
    exec_mode = int(mode_input) if mode_input in ["1", "2", "3"] else 1
    ...
    print("-" * 70)
    print(f"[OK] Configuration Locked: {threads} Threads | {ram} GB RAM | Mode {exec_mode} | Auto-Push: {auto_push}")
    print("=" * 70 + "\n")
    return threads, ram, exec_mode, auto_push

def setup_case_directory(case_dir, case_meta, fluid_props, num_procs):
    ...
    q_flux = case_meta["tdp_per_chip_W"] / 0.0056 # 80x70 mm base
    ...
    generate_blockmesh_dict(case_dir, open_ratio=case_meta["open_ratio"], topology=case_meta["topology"])
...
def run_single_simulation(case_dir, case_meta, fluid_db, num_procs):
    ...
    if num_procs > 1:
        ...
        mpi_cmd = f"mpirun --use-hwthread-cpus -np {num_procs} simpleFoam -parallel > {log_file} 2>&1"
...
def main():
    ...
    hw = env["hardware"]
    threads, ram, exec_mode, auto_push = interactive_setup(hw, args)
    ...
    print(f"      Threads: {threads} | Memory Limit: {ram} GB | Auto-Push: {auto_push}")
    print("=" * 70)
    ...
    for idx, case_meta in enumerate(cases_to_run, 1):
        ...
        print(f"\n[Case {idx:02d}/{total_cases:02d}] Launching {cid} ({group}):")
        ...
        setup_case_directory(case_dir, case_meta, fluid_db[fluid], num_procs=threads)
        
        try:
            res = run_single_simulation(case_dir, case_meta, fluid_db, num_procs=threads)

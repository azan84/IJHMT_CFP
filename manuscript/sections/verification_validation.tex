...
\begin{table}[htbp]
\centering
\caption{Validation comparisons against anchor-tier literature benchmarks.}
\label{tab:vv-anchortier}
\scriptsize
\sloppy
\begin{tabular}{p{0.18\textwidth}p{0.25\textwidth}p{0.22\textwidth}p{0.27\textwidth}}
\hline
Case (tier) & Reported comparison quantity & Source uncertainty & Present OpenFOAM deviation \\
\hline
Dong et al.~\cite{dong2026} (ANCHOR-CONFIRMED) & CPU junction temperature vs. flow rate ($Q = 2.0\text{--}8.0$~L/min) at 1.75~kW in 1U immersion chassis with EFL-1 & 3.0\% max dev. & $T_j = 43.4\text{--}73.8$~$^{\circ}$C (MAPE = 1.20\%, max dev. 2.97\%); confirms full-scale chassis CHT. \\
Kewalramani et al.~\cite{kewalramani2019} (ANCHOR-ACQUIRABLE) & Outlet temperature and pressure drop for $5\times5$ micro-pin-fin array & $\sim 3\%$ on $T$; 2.41\% on $\Delta p$ & $\Delta p_{\mathrm{core}} = 2.43\text{--}2.50$~kPa (within 2.1\% of analytical loss); thermal dev. $<1.8\%$ across solid-fluid interface. \\
Sastre et al.~\cite{sastre2018} (ANCHOR-ACQUIRABLE) & Pressure drop and tip bypass diversion ($c/H_f = 0, 0.5$, $Q = 53.5\text{--}167.2$~L/h) & 3.6--7.7\% on $\Delta p$ & $\Delta p$ reduction MAPE = 12.2\% (ratio MAPE 21.6\% across 4 flow rates); confirms bypass diversion mechanism. \\
Khoshvaght-Aliabadi et al.~\cite{khoshvaghtaliabadi2026oblique} (ANCHOR-ACQUIRABLE) & $R_{\mathrm{th}}$ and $T_{\mathrm{CPU}}$ across $P = 200\text{--}600$~W in 1U chassis with FC-40 & $<3.4\%$ on $R_{\mathrm{th}}$, exp. unc. 6.8\% & $R_{\mathrm{th}} = 0.0805\text{--}0.0872$~K/W (MAPE = 0.67\%, max dev. 0.91\%, $\Delta T < 0.35^\circ$C); validates oblique-fin mixing. \\
\hline
\end{tabular}
\end{table}
